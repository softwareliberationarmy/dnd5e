...
{
    public class CharacterSavingThrowHandlerTests: TestBase
    {
        [Fact]
        public async Task ShouldParseRequestAndRollForCharacter()
        {
            var characterId = 12;
            var rollType = "constitution";
            ...
            var target = Mocker.CreateInstance<CharacterSavingThrow.Handler>();
            var result = await target.Handle(new CharacterSavingThrow.Request(characterId, rollType), CancellationToken.None);
        ...
        public async Task ShouldThrowNotFoundExceptionWhenCharacterNotFound()
        {
            ...
            (await target.Invoking(
                    t => t.Handle(new CharacterSavingThrow.Request(characterId, "strength"), CancellationToken.None))
                .Should()
        ...
        public async Task ShouldThrowNotFoundExceptionWhenAbilityNotFound()
        {
            ...
            var target = Mocker.CreateInstance<CharacterSavingThrow.Handler>();
            (await target.Invoking(
                    t => t.Handle(new CharacterSavingThrow.Request(characterId, "parcheesi"), CancellationToken.None))
                .Should()
